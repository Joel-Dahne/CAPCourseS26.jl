...
computer-assisted proof for the self-similar blowup scenario for the CGL
...
version of ``G``, as well as a multi-dimensional version of the
...
won't have the time to properly implement rigorous evaluations of
...
implementation of everything else. We will roughly follow this
order:

1. Skeletons of ``Q_0`` and ``Q_\infty``
2. Implementation of ``G``
3. Start implementing interval Newton
4. Skeletons of Jacobian of ``Q_0`` and ``Q_\infty``
5. Implementation of Jacobian of ``G``
6. Finish implementing interval Newton
...
this. To begin with, we will implement non-rigorous versions of the
