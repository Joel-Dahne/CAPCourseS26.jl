...
md"""
# Lab 11: Self-similar blowup - again!
"""

# ╔═╡ 65d5bf11-ac8c-4888-9d2f-64f824233f0c
md"""
## Some Arblib.jl things

Fix some issues with Arblib and wrap some functions (you don't have to care too much about this).
"""

# ╔═╡ 3454dddd-4941-49dc-9032-9094b470fc48
# This make it so that operations on `Arb` and `Complex` give `Acb`. This should probably be in Arblib, but it is not...
begin
...
# ╔═╡ 3384e3e0-c681-4c28-ba03-c59566d8e46d
begin
...
md"""
## Parameters
These are the parameters for an approximate zero of ``G``.
"""
...
γ_0 = Acb(1.71360, -1.49179)

# ╔═╡ c3c30b25-1cbe-4d53-8bd0-50f0b1b1893b
κ_0 = Arb(0.91735)

# ╔═╡ d2d81e7b-ea2d-4ef8-b368-cd36feb386e4
md"""
## Implementation of ``Q_0``

To handle the removable singularity of ``Q_0(\xi)`` at zero we split the implementation into two parts. One using a Taylor expansion centered at zero, and one using a standard ODE solver.

Note that we are here using a non-rigorous ODE solver which does all the computations in `Float64`. The general idea is the same for the rigorous version as well, but it requires external libraries to implement.
"""

# ╔═╡ 840dbb7f-648c-4da5-89c2-af461d319ced
function Q_zero_taylor(μ::Arb, κ::Arb, ξ₀::Arb; N = 20)
    ...
    add_error(a₀, remainder_bound)
    add_error(b₀, remainder_bound)
    add_error(a₁, remainder_dξ_bound)
    add_error(b₁, remainder_dξ_bound)

    return SVector(Acb(a₀, b₀), Acb(a₁, b₁))
end

# ╔═╡ a02bd9de-0807-485f-b665-947e7fe4d954
function nls_equation_real_system(Q::SVector{4}, κ, ξ::Float64)
    a, b, a_dξ, b_dξ = Q

    a^2 + b^2

    a_dξ_dξ = κ * ξ * b_dξ + κ * b + a - (a^2 + b^2) * a
    b_dξ_dξ = -κ * ξ * a_dξ - κ * a + b - (a^2 + b^2) * b
    ...
    return SVector(a_dξ, b_dξ, a_dξ_dξ, b_dξ_dξ)
end

# ╔═╡ cac38a47-cc72-4668-aa1c-c3f2ed2e9981
function Q_zero_float(Q_ξ₀::SVector{4}, κ, ξ₀::Float64, ξ₁::Float64)
    prob = ODEProblem{false}(nls_equation_real_system, Q_ξ₀, (ξ₀, ξ₁), κ)

    return solve(prob, Vern7(), abstol = 1e-11, reltol = 1e-11)[end]
end

# ╔═╡ 6b1f8d2a-cb1d-465e-9d1f-82d7b705e68d
function Q_zero_float(Q_ξ₀::SVector{2,Acb}, κ::Arb, ξ₀::Arb, ξ₁::Arb)
    Q_ξ₀_float =
        SVector{4,Float64}(real(Q_ξ₀[1]), imag(Q_ξ₀[1]), real(Q_ξ₀[2]), imag(Q_ξ₀[2]))

    Q_ξ₁_float = Q_zero_float(Q_ξ₀_float, Float64(κ), Float64(ξ₀), Float64(ξ₁))

    return SVector{2,Acb}(
        Acb(Q_ξ₁_float[1], Q_ξ₁_float[2]),
        Acb(Q_ξ₁_float[3], Q_ξ₁_float[4]),
    )
end
...
function Q_zero(μ::Arb, κ::Arb, ξ₁::Arb)
    ...
    # FIXME: This part is not rigorous!
    return Q_zero_float(Q_ξ₀, κ, ξ₀, ξ₁)
end

# ╔═╡ 4f4c61ab-69bf-42ed-bad9-2a09b43e7e1a
Q_zero(μ_0, κ_0, ξ₁)

# ╔═╡ d1608ed5-69e6-4014-963d-188407c74bf0
ComplexF64.(Q_zero(μ_0, κ_0, ξ₁))

# ╔═╡ 00bb87be-51d5-4b35-8004-a52c85bd4541
md"""
For computing the Jacobian with respect to ``\mu`` and ``\kappa`` we should also split into one part using a Taylor expansion and one part using a regular ODE solver. However, the implementation is a little bit more tricky in this case, so we fall back to using the non-rigorous ODE solver for the entire interval (this wouldn't work for the rigorous version).
"""

# ╔═╡ fec73370-c096-45fd-8b08-94d7284da80f
function Q_zero_jacobian_float(μ::Float64, κ::Float64, ξ₁::Float64)
    ξ₀ = 0.0
    return DifferentiationInterface.jacobian(AutoForwardDiff(), SVector(μ, κ)) do (μ, κ)
        Q_zero_float(SVector(μ, 0, 0, 0), κ, ξ₀, ξ₁)
    end
end

# ╔═╡ b175ca4c-2dca-48a9-941f-2b848c5ba57d
function Q_zero_jacobian_float(μ::Arb, κ::Arb, ξ₁::Arb)
    J_float = Q_zero_jacobian_float(Float64(μ), Float64(κ), Float64(ξ₁))
    return SMatrix{2,2,Acb}(
        # Derivatives with respect to μ
        Acb(J_float[1, 1], J_float[2, 1]),
        Acb(J_float[3, 1], J_float[4, 1]),
        # Derivatives with respect to κ
        Acb(J_float[1, 2], J_float[2, 2]),
        Acb(J_float[3, 2], J_float[4, 2]),
    )
end
...
function Q_zero_jacobian(μ::Arb, κ::Arb, ξ₁::Arb)
    ...
    return Q_zero_jacobian_float(μ, κ, ξ₁)
end

# ╔═╡ 8ce3188a-9f33-4ed6-909c-a0b5a39d2410
Q_zero_jacobian(μ_0, κ_0, ξ₁)

# ╔═╡ 900aaa6d-a1cf-4608-b04a-1187d9ad38f7
ComplexF64.(Q_zero_jacobian(μ_0, κ_0, ξ₁))

# ╔═╡ 18eed5da-afee-4fa4-9032-45613ceddfd7
md"""
## Implementation of ``Q_\infty``
"""

# ╔═╡ e76e2608-032e-445b-855a-4f63de202969
md"""
### Implementations of ``U``

Flint implements the hypergeometric function ``U``, but it doesn't implement any derivatives for it. In particular the derivative with respect to ``a`` requires quite a bit of work to do rigorously. Here we just take the implementations for granted.

The first implementation is for direct evaluation. It just wraps Arblib.jl.
"""

# ╔═╡ 96de753a-0f6a-474d-ad0b-7c07921698cf
U(a::Acb, b::Acb, z::Acb) = Arblib.hypgeom_u!(zero(z), a, b, z)

# ╔═╡ 36f24627-cd90-46b7-9d2a-6d8e9b646dc6
md"""
For derivatives with respect to ``z`` we implement a verson taking `z::AcbSeries` as an argument. The implementation doesn't matter too much for us here.
"""

# ╔═╡ 495bc4a4-da35-4c3c-9514-e272f4c51c21
md"""
For derivatives with respect to ``a`` and ``z`` simultaniously we implement a version taking `a::AcbSeries` and `z::AcbSeries`. This is only implemented for degree 1 series.
"""

# ╔═╡ 3d31331b-62ce-445f-8914-7e9a79a75ea3
...
# ╔═╡ 06bd91fe-4ce7-4d1f-99a8-b0db6c3bd980
...
# ╔═╡ 65c16e79-e584-42eb-838a-90d4182ec5e6
...
# ╔═╡ 4c433b4b-d586-4e8f-8ff6-0a7b5e8ea84e
...
# ╔═╡ 6aaef50f-9a1c-4aa2-b02a-1a8a84947ddd
...
# ╔═╡ 69b5bc61-a5ec-49c3-8051-70eb451a6670
...
# ╔═╡ d4fb706a-08dd-4c2f-99a9-a8432e2767ab
function U(a::AcbSeries, b::Acb, z::AcbSeries)
    Arblib.degree(a) == Arblib.degree(z) == 1 ||
        throw(ArgumentError("only supports degree 1"))

    AcbSeries((U(a[0], b, z[0]), U_da(a[0], b, z[0]) * a[1] + U_dz(a[0], b, z[0]) * z[1]),)
end

# ╔═╡ 4a0917a8-c48e-430e-9c8f-ad643229fe9a
...
# ╔═╡ 8f437b5c-e0d4-4268-bd39-e9734b1c250d
function U(a::Acb, b::Acb, z::AcbSeries)
    z₀ = z[0]
    res = zero(z)
...
# ╔═╡ 4bf7d51e-c34b-4f1d-ad63-eb9acf9a2aa4
md"""
### Implementations of ``P`` and ``E``

Next we implement ``P`` and ``E``. Here it is your task to fill out some of the details. See Week 11 Lecture 2.
"""
...
# ╔═╡ 1d14a431-269d-4637-8d41-e7aae8fabcb2
function P_dξ(κ, ξ)
    # For κ::Arb and ξ::Arb this could be implemented as
    #return P(κ, ArbSeries((ξ, 1)))[1]
...
# ╔═╡ d547d9f2-d888-4357-bfa3-8e5d106bd9c2
function P_dκ(κ::Arb, ξ::Arb)
    return P(ArbSeries((κ, 1)), ξ)[1]
end

# ╔═╡ 3ee272de-11c7-4b28-bd47-07d73ba43b94
function P_dξ_dκ(κ::Arb, ξ::Arb)
    return P_dξ(ArbSeries((κ, 1)), ξ)[1]
end

# ╔═╡ e3dc42ac-d152-47dd-a6df-d7d21ad8c9ad
P(κ_0, ξ₁)

# ╔═╡ 89f209fe-3f3c-443b-805d-2b468b65d085
P_dξ(κ_0, ξ₁)

# ╔═╡ 6f4ef0e8-df53-4599-a9e7-0870e0121f3a
P_dκ(κ_0, ξ₁)

# ╔═╡ d8d85ccf-0c71-4c42-b47c-9129f573ec39
P_dξ_dκ(κ_0, ξ₁)
...
# ╔═╡ 188e29f0-acc7-4b27-a4e7-effcde19df8d
function E_dξ(κ, ξ)
    # For κ::Arb and ξ::Arb this could be implemented as
    #return E(κ, ArbSeries((ξ, 1)))[1]
    # However, that doesn't work for κ::ArbSeries. We hence
    # give a manual implementation.
...
# ╔═╡ cf53ce5b-f4c9-44b9-9d8d-753d81985371
function E_dκ(κ::Arb, ξ::Arb)
    return E(ArbSeries((κ, 1)), ξ)[1]
end

# ╔═╡ f4bacfbb-16dd-41af-aefd-258e5d772142
function E_dξ_dκ(κ::Arb, ξ::Arb)
    return E_dξ(ArbSeries((κ, 1)), ξ)[1]
end

# ╔═╡ c8a75e94-5ddb-44fb-8712-ed69e078d343
md"""
### Implementation of ``Q_\infty``
"""
...
function Q_infty(γ::Acb, κ::Arb, ξ₁::Arb)
    I_E = zero(ξ₁) # NOTE: This should be zero!
    I_P = zero(ξ₁) # FIXME: This should not be zero!
    ...
    return SVector(Q, Q_dξ)
end

# ╔═╡ f225fdb3-db78-45b6-a152-6e29a7b777d7
Q_infty(γ_0, κ_0, ξ₁)

# ╔═╡ 7af3d8f5-1c92-4b2a-ae20-ea34ca3b8e19
ComplexF64.(Q_infty(γ_0, κ_0, ξ₁))

# ╔═╡ 13aa1b0d-cb5a-404d-b8ad-20ac4301b173
function Q_infty_jacobian(γ::Acb, κ::Arb, ξ₁::Arb)
    I_E = zero(ξ₁) # NOTE: This should be zero!
    I_P = zero(ξ₁) # FIXME: This should not be zero!
    I_E_dγ = zero(ξ₁) # NOTE: This should be zero!
    I_P_dγ = zero(ξ₁) # FIXME: This should not be zero!
    I_E_dκ = zero(ξ₁) # NOTE: This should be zero!
    I_P_dκ = zero(ξ₁) # FIXME: This should not be zero!

    I_E_dξ = zero(ξ₁) # FIXME: This should not be zero!
    I_P_dξ = zero(ξ₁) # FIXME: This should not be zero!
    I_E_dξ_dγ = zero(ξ₁) # FIXME: This should not be zero!
    I_P_dξ_dγ = zero(ξ₁) # FIXME: This should not be zero!
    I_E_dξ_dκ = zero(ξ₁) # FIXME: This should not be zero!
    I_P_dξ_dκ = zero(ξ₁) # FIXME: This should not be zero!

    Q_dγ = P(κ, ξ₁) + P(κ, ξ₁) * I_E_dγ + E(κ, ξ₁) * I_P_dγ
    ...
    Q_dκ = γ * P_dκ(κ, ξ₁) + P_dκ(κ, ξ₁) * I_E + P(κ, ξ₁) * I_E_dκ
    E_dκ(κ, ξ₁) * I_P + E(κ, ξ₁) * I_P_dκ
    ...
    return SMatrix{2,2}(Q_dγ, Q_dξ_dγ, Q_dκ, Q_dξ_dκ)
end

# ╔═╡ 6066ed44-7ce4-4f69-b47c-26e69bc74be8
Q_infty_jacobian(γ_0, κ_0, ξ₁)

# ╔═╡ 3c8960b7-7d98-4402-b2ff-6d2a3973feeb
ComplexF64.(Q_infty_jacobian(γ_0, κ_0, ξ₁))
...
function G(μ::Arb, γ_real::Arb, γ_imag::Arb, κ::Arb, ξ₁::Arb)
    res = Q_zero(μ, κ, ξ₁) - Q_infty(Acb(γ_real, γ_imag), κ, ξ₁)
    return SVector(real(res[1]), imag(res[1]), real(res[2]), imag(res[2]))
end

# ╔═╡ 8accbcc0-3b37-47cd-a3f7-db29ce0f6fcb
G(μ_0, real(γ_0), imag(γ_0), κ_0, ξ₁)

# ╔═╡ 1abe87c7-0b7e-44f5-b21d-902be3d52acd
Float64.(G(μ_0, real(γ_0), imag(γ_0), κ_0, ξ₁))

# ╔═╡ 7f20a7c1-6382-4db9-8c38-d3d404c8af8a
function G_jacobian(μ::Arb, γ_real::Arb, γ_imag::Arb, κ::Arb, ξ₁::Arb)
    Q_0_J = Q_zero_jacobian(μ, κ, ξ₁)
    Q_inf_J = Q_infty_jacobian(Acb(γ_real, γ_imag), κ, ξ₁)
    return SMatrix{4,4,Arb}(
        # Derivatives w.r.t. μ
        real(Q_0_J[1, 1]),
        imag(Q_0_J[1, 1]),
        real(Q_0_J[2, 1]),
...
G_jacobian(μ_0, real(γ_0), imag(γ_0), κ_0, ξ₁)

# ╔═╡ ccd8fabd-2696-479a-a991-6d0d73bf6a51
Float64.(G_jacobian(μ_0, real(γ_0), imag(γ_0), κ_0, ξ₁))

# ╔═╡ 3bbff377-e805-478e-9dd4-578a9e002fc4
md"""
## Implementation of interval Newton
"""

# ╔═╡ 66dbee77-5060-4f41-830f-29ec3be7ed85
function interval_newton_step(x::SVector{4,Arb})
    m = midpoint.(Arb, x)
...
# ╔═╡ b07421c5-7235-4ab6-bf7c-785d3e1174be
md"""
We first manually run a couple of non-rigorous Newton iterations to improve our approximation.
"""

# ╔═╡ f2dcb42f-4826-46be-bc34-f70031469a68
x_0 = SVector(μ_0, real(γ_0), imag(γ_0), κ_0)

# ╔═╡ 1a2038b2-a7da-48a5-901e-f6ede4574511
G(x_0..., ξ₁)

# ╔═╡ 43f181c7-a83f-41ec-a5cd-5de3a8854e7d
x_1 = interval_newton_step(x_0)

# ╔═╡ 72b8055a-07aa-4790-b907-b5c5e2d52f00
G(x_1..., ξ₁)

# ╔═╡ 535c5e37-d0e2-4252-8c79-28c9ab33dda1
x_2 = interval_newton_step(x_1)

# ╔═╡ 803be92a-2b68-450a-8b76-0cb2f2290ab5
G(x_2..., ξ₁)

# ╔═╡ e7a6f7c2-f72e-43a0-a9fa-8e8176cf0f32
md"""
At this point we have a sufficiently good approximation. We take a small box around it and apply the interval Newton method to this box.
"""

# ╔═╡ e97c5b70-cad9-4892-bdc7-235c26a2bd67
x_2_box = add_error.(x_2, Mag(1e-8))

# ╔═╡ 48e9afb4-671d-4734-9aef-d254e88e8fd7
Nx_2_box = interval_newton_step(x_2_box)

# ╔═╡ f696af79-e2cd-4b44-bba9-564b42f30549
Arblib.contains.(x_2_box, Nx_2_box)

# ╔═╡ 38909db4-57e0-42a9-9660-129bcd09abc5
if all(Arblib.contains.(x_2_box, Nx_2_box))
    @info "Success!"
else
    @warn "Failure!"
end
...
manifest_format = "2.0"
project_hash = "7975ae89edc26d2b779c68bd391eb9ec6e58195a"
...
git-tree-sha1 = "28ddfd7d136564c5b2a09d28759feccc2005dbea"
