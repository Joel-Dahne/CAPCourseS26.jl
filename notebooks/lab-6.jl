...
# ╔═╡ 455617a6-1404-11f1-87b9-61ab55cb9536
using Arblib, IntervalArithmetic, Plots

# ╔═╡ f7f210be-b9e1-4aa3-9d47-cae04d468536
md"""
# Lab 6

This notebook contains the second part of Lab 6. See the package documentation for the full instructions.
"""

# ╔═╡ 8449b204-a1df-4443-9b2a-96b0ec98f750
setdisplay(:full) # To see the full output for intervals

# ╔═╡ 8cd73243-e4f8-4db7-9c93-19d43adb6e3c
md"""
## Computing ``\sin``
...
# ╔═╡ 7d0595e8-c14e-4c5b-abb9-0eddc94d3c83
...
# ╔═╡ 71486bbb-845a-45ad-8d6e-b75dc717bec4
md"""
Let us check that it seems to work
"""

# ╔═╡ 83d990d8-11ce-4ffd-91b3-18bb55a026db
sin(1.0)

# ╔═╡ 1549b6da-0a07-47aa-9dc9-e6b89212290b
sin(10.0)

# ╔═╡ f669fb35-2bb5-4ced-96f3-26786f5ae230
md"""
...
# ╔═╡ be19dbf8-f6f9-4e7e-94ec-bbf89e118ee8
function my_sin(x::Interval{Float64}; N::Integer = 6)
    # TASK: Implement this
end

# ╔═╡ aaa4e5e8-4a81-479c-91fb-fb0d114fb339
function my_sin(x::Arb; N::Integer = 6)
    # TASK: Implement this
end

# ╔═╡ f01a21c2-e43f-4b8e-980b-67b2526ff1b7
my_sin(1.0)

# ╔═╡ 5ae25be6-7497-4561-8149-a1adb2dbb1b7
my_sin(1.0) ≈ sin(1.0)

# ╔═╡ e6c28a65-8d5b-4445-9614-42242c54102b
my_sin(10.0) # For large values of x we would need more terms

# ╔═╡ 3bbdb729-0397-41c8-99c8-2e6adc75f7d1
md"""
We can check if it seems to work.
"""

# ╔═╡ a59b4596-2df1-4ca4-a39b-f4ded3e96673
my_sin(interval(1))

# ╔═╡ 313c9421-05b8-4f5d-aec8-044ce7c92178
sin(interval(1))

# ╔═╡ c080d3a2-92fd-47db-a0b5-b3b2de7306d3
issubset_interval(sin(interval(1)), my_sin(interval(1)))

# ╔═╡ a9d2d98a-1f7a-4985-bf87-9415d32b130c
my_sin(Arb(1))

# ╔═╡ 5014153e-749b-4e4a-832d-376171f2920f
sin(Arb(1))

# ╔═╡ c41ea08a-3c5f-4cc5-8bb9-aa6e4656b307
Arblib.contains(my_sin(Arb(1)), sin(Arb(1)))

# ╔═╡ 7f4fe17a-0732-4d21-8e6e-ce67b5d2e82d
my_sin(interval(10)) # For large values of x we get enormous error bounds

# ╔═╡ dcc69e1b-7a86-4256-a191-1a1927cc7a5f
my_sin(Arb(10))

# ╔═╡ c403bbeb-2d5e-4553-a5a5-655f6679d5c2
md"""
...
# ╔═╡ 3bf332af-08e2-4ddc-8adc-68d5603cf3d6
md"""
## Bonus: Wide versus thin cutoff for ``\sin``

In the last lecture we discussed the conceptual differences between wide and thin interval arithmetic. Let us take a look at how this is handled in practice in the Arb implementation of ``\sin``.

For this we will look at balls with radius ranging from ``10^{-20}`` to ``10`` on logarithmic scale.
"""

# ╔═╡ df62cc9a-7e4f-4f3e-958e-05d27d0b4795
rs = logrange(1e-20, 10, 1000)

# ╔═╡ 3b6b94dd-6827-4fe2-b1c2-bcf56a1665bd
xs = setball.(Arb, 1, rs)

# ╔═╡ cfcc3397-a512-4082-a16d-4316c7348dbd
ys = sin.(xs)

# ╔═╡ 81d2a4e7-99cd-45da-8a24-470fd47d80c6
md"""
In this plot we can see when the implementation switches from "thin" interval arithmetic to "wide" interval arithmetic as the radius becomes larger. Can you see where?
"""

# ╔═╡ 6bf83107-48e7-432f-9210-b710fcef72cc
let
    plot(
        rs,
        abs.(midpoint.(ys) .- sin(Arb(1))),
        xlabel = "r",
        xscale = :log10,
        yscale = :log10,
        label = "Midpoint error",
        legend = :topleft,
    )
    plot!(rs, Float64.(Arblib.radius.(ys)), label = "Radius")
end
...
manifest_format = "2.0"
project_hash = "e307f6c3dbb6bb432aa4bc6dee8122cb0ca9bd45"
